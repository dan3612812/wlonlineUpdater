import threading
import logging
from ftplib import FTP
import socket
import time
from datetime import datetime
from .conf import dateFormat, FullDatetimeFormat, programPath


class FileInfo:
    type: str
    size: int
    modify: FullDatetimeFormat
    perms: str
    filename: str


def setInterval(interval, times=-1):
    # This will be the actual decorator,
    # with fixed interval and times parameter
    def outer_wrap(function):
        # This will be the function to be
        # called
        def wrap(*args, **kwargs):
            stop = threading.Event()

            # This is another function to be executed
            # in a different thread to simulate setInterval
            def inner_wrap():
                i = 0
                while i != times and not stop.isSet():
                    stop.wait(interval)
                    function(*args, **kwargs)
                    i += 1

            t = threading.Timer(0, inner_wrap)
            t.daemon = True
            t.start()
            return stop
        return wrap
    return outer_wrap


class PyFTPclient:
    def __init__(self, host, port=21, login='anonymous', passwd='anonymous', monitor_interval=30):
        self.host = host
        self.port = port
        self.login = login
        self.passwd = passwd
        self.monitor_interval = monitor_interval
        self.ptr = None
        self.max_attempts = 15
        self.waiting = True
        self.ftp = self.__ftpInit()

    def __ftpInit(self):
        ftp = FTP()
        ftp.connect(self.host, self.port)
        ftp.login(self.login, self.passwd)
        # optimize socket params for download task
        ftp.sock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
        ftp.sock.setsockopt(socket.IPPROTO_TCP,
                            socket.TCP_KEEPINTVL, 75)
        ftp.sock.setsockopt(socket.IPPROTO_TCP,
                            socket.TCP_KEEPIDLE, 60)
        ftp.set_debuglevel(2)
        ftp.set_pasv(True)
        # ftp.voidcmd('TYPE I')

        return ftp

    def parseToISO(self, str: str) -> FullDatetimeFormat:
        year = str[:4:]
        mouth = str[4:6:]
        day = str[6:8:]
        hour = str[8:10:]
        minute = str[10:12:]
        second = str[12:14:]
        ms = str[15:]

        return "%s-%s-%sT%s:%s:%s.%s" % (year, mouth, day, hour, minute, second, ms)

    def dir(self):
        ls: list[str] = []
        self.ftp.retrlines("MLSD", ls.append)
        fileInfos: list[FileInfo] = []
        for line in ls:
            fileInfo: FileInfo = {
                "filename": line.split(";")[-1].replace(" ", "")
            }
            for st in line.split(";"):
                if "type=" in st:
                    fileInfo["type"] = st[5:]
                if "size=" in st:
                    fileInfo["size"] = int(st[5:])
                if "modify=" in st:
                    fileInfo["modify"] = self.parseToISO(st[7:])
                if "perms=" in st:
                    fileInfo["perms"] = st[6:]
            fileInfos.append(fileInfo)

        fileInfos.sort(key=lambda obj: obj["modify"], reverse=True)
        return fileInfos

    def downloadFile(self, dst_filename, local_filename=None):
        res = ''
        if local_filename is None:
            local_filename = dst_filename

        with open(local_filename, 'wb') as f:
            self.ptr = f.tell()

            @setInterval(self.monitor_interval)
            def monitor():
                if not self.waiting:
                    i = f.tell()
                    if self.ptr < i:
                        logging.debug("%d  -  %0.1f Kb/s" %
                                      (i, (i-self.ptr)/(1024*self.monitor_interval)))
                        self.ptr = i
                    else:
                        self.ftp.close()

            dst_filesize = self.ftp.size(dst_filename)

            mon = monitor()
            while dst_filesize > f.tell():
                try:
                    self.__ftpInit()
                    self.waiting = False
                    # retrieve file from position where we were disconnected
                    res = self.ftp.retrbinary('RETR %s' % dst_filename, f.write) if f.tell() == 0 else \
                        self.ftp.retrbinary('RETR %s' % dst_filename,
                                            f.write, rest=f.tell())

                except:
                    self.max_attempts -= 1
                    if self.max_attempts == 0:
                        mon.set()
                        logging.exception('')
                        raise
                    self.waiting = True
                    logging.info('waiting 30 sec...')
                    time.sleep(30)
                    logging.info('reconnect')

            mon.set()  # stop monitor
            self.ftp.close()

            if not res.startswith('226 Transfer complete'):
                logging.error(
                    'Downloaded file {0} is not full.'.format(dst_filename))
                # os.remove(local_filename)
                return None

            return 1
